...
export default function FlowBar({ status, onStatusChange }: FlowBarProps) {
  ...
  const MaskedSVG = ({ src, color }: { src: string; color: string }) => (
    <div
      className="w-[262px] h-[50px] sm:w-[200px] sm:h-[40px]" // responsive size
      style={{
  ...
  return (
    <div className="flex flex-wrap justify-center gap-2 max-w-[1000px] mx-auto">
      {/* Step 1: New */}
      <div className="relative w-[262px] sm:w-[200px] -mr-14 sm:-mr-8">
        <MaskedSVG src="/images/sales/flow_bar1.svg" color={getColor("New")} />
        <span className="absolute inset-0 flex items-center justify-center text-white font-semibold text-[16px] sm:text-[14px]">
          New
        </span>
      </div>

      {/* Step 2: Ongoing */}
      <div className="relative w-[262px] sm:w-[200px] -mr-[54px] sm:-mr-8">
        <MaskedSVG
      ...
      {/* Step 3: Won/Lost */}
      <div className="relative w-[262px] sm:w-[200px]">
        <MaskedSVG
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="relative">
            <select
              ...
              className={`appearance-none bg-white text-black font-semibold text-[16px] sm:text-[14px] pr-6 cursor-pointer rounded-md py-1 pl-2 focus:outline-none focus:ring-2 focus:ring-red-500 ${
            ...
            <ChevronDown className="absolute right-1 top-1/2 -translate-y-1/2 text-black w-4 h-4 pointer-events-none" />
          </div>
        </div>
      </div>
    </div>
